...
from mmflow.datasets.builder import DATASETS
...
class CrowdFlow(BaseDataset):
    ...
    def load_data_info(self) -> None:
        ...
        # unpack CrowdFlow directly, will see `data` subdirctory.
        ...
        # data in CrowdFlow dataset has specific suffix
        self.img1_suffix = "_img1.ppm"
        self.img2_suffix = "_img2.ppm"
        self.flow_suffix = "_flow.flo"

        img1_filenames = self.get_data_filename(self.img1_dir, self.img1_suffix)
        img2_filenames = self.get_data_filename(self.img2_dir, self.img2_suffix)
        flow_filenames = self.get_data_filename(self.flow_dir, self.flow_suffix)

        assert len(img1_filenames) == len(img2_filenames) == len(flow_filenames)

        self.load_img_info(img1_filenames, img2_filenames)
        self.load_ann_info(flow_filenames, "filename_flow")

    def load_img_info(
        self, img1_filename: Sequence[str], img2_filename: Sequence[str]
    ) -> None:
        ...
        for i in range(num_file):
            if (not self.test_mode and self.split[i] == 1) or (
                self.test_mode and self.split[i] == 2
            ):
                data_info = dict(
                    img_info=dict(
    ...
    def load_ann_info(self, filename: Sequence[str], filename_key: str) -> None:
        ...
        for i in range(num_files):
            if (not self.test_mode and self.split[i] == 1) or (
                ...
                self.data_infos[i - num_tests]["ann_info"][filename_key] = filename[i]
            else:
                num_tests += 1
